...
int serialport_good = 0;                              /* Used to toggle LED */
char bufferIn[3];
int bufferIn_Count;
int bufferInMax = 4;
int signal_received = 0;                              /* Used for shutdown */
int led_on = 0;                                       /* Used for LED toggle */
HANDLE hserial;
...
void get_port_name()                                  /* Get Com Port # */
{
    ...
    while((inputChar = getchar()) != '\n')
    {
        if (!isdigit(inputChar))                      /* Check if a digit */
        {
            ...
            printf("  Press Enter\n");
            getchar();
            getchar();                                /* Add delay to display error */
            exit(0);                                  /* Exit program */
        }

        if (bufferIn_Count < bufferInMax)            /* If there is room in the buffer? */
        {
